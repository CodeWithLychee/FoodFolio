...
const registerStudent = async (req, res) => {
  ...
  try {
    ...
    let student = await Student.findOne({ mess_id });
    if (student) {
      return res
        .status(400)
    ...
    // Add student to the hostel's students list
    shostel.students.push(student._id);
    await shostel.save();
    ...
    res
      .status(500)
      .json({ success, errors: [{ msg: `Server error: ${err.message}` }] });
  }
};
